...
def init_db():
    ...
    con.execute(
        "ALTER TABLE passengers ADD COLUMN IF NOT EXISTS submission_id TEXT"
    )
...
def availability(program, dt):
    ...
    remaining = max(
        0,
        capacity - booked_people(program, dt)
    )
...
Sitemap: https://hunter-booking.onrender.com/sitemap.xml
"""
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    <url>
        <loc>https://hunter-booking.onrender.com/</loc>
...
def reserve():
    ...
    sms(
        ...
        f"예금주 {BANK_HOLDER}\n"
        f"입금 시 날짜+예약자명으로 입금해주세요.\n"
        f"입금 확인 후 예약확정 안내드립니다."
    )
...
def passenger():
    ...
    boarding_date = request.form.get(
        "boarding_date",
        ""
    ).strip()
    ...
    birth_dates = request.form.getlist(
        "birth_date[]"
    )

    phones = request.form.getlist(
        "phone[]"
    )

    emergency_phones = request.form.getlist(
        "emergency_phone[]"
    )
    ...
    for i in range(max_len):
        ...
        if (
            len(birth_date) != 8
            or not birth_date.isdigit()
        ):

            flash(
                f"{i + 1}번 승선자 생년월일은 8자리 숫자로 입력해주세요."
            )
        ...
        if (
            not emergency_phone.isdigit()
            or len(emergency_phone) < 10
        ):

            flash(
                f"{i + 1}번 승선자 비상연락처를 다시 확인해주세요."
            )
    ...
    if booking and len(rows) != int(
        booking["people"]
    ):

        flash(
            f"예약 인원은 {booking['people']}명입니다. "
            f"현재 {len(rows)}명 입력되어 있습니다. "
            f"승선자 전원을 입력해주세요."
        )
    ...
    try:
        ...
        if booking_id:
            ...
            con.execute(
                "DELETE FROM passengers WHERE booking_id = %s",
                (booking_id,)
            )
        ...
        flash(
            "승선명부 저장 중 오류가 발생했습니다. 다시 시도해주세요."
        )
...
def manual_booking():
    ...
    if people > remaining:
        ...
        flash(
            f"수동 예약 등록 불가: 현재 잔여 인원은 {remaining}명입니다."
        )
    ...
    try:
        ...
        if schedule:
            ...
            if schedule["state"] != "예약가능":
                ...
                flash(
                    f"{dt} {program}은 현재 {schedule['state']} 상태입니다."
                )
        ...
        if people > remaining_locked:
            ...
            flash(
                f"수동 예약 등록 불가: 현재 잔여 인원은 {remaining_locked}명입니다."
            )
...
def set_status(bid, status):
    ...
    if status != old_status:
        ...
        if status == "입금확인":
            ...
            manifest_url = (
                "https://hunter-booking.onrender.com/"
                f"passenger?booking_id={bid}"
            )
            ...
            sms(
                ...
                f"{dt} {program}\n"
                f"예약이 확정되었습니다.\n"
                f"아래 링크에서 승선자 전원의 승선명부를 작성해주세요.\n"
                f"{manifest_url}\n"
                f"감사합니다."
            )
