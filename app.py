...
def get_schedule(program, date):
    ...
    if row:
        return dict(row)
    return {"program": program, "date": date, "capacity": PROGRAMS[program]["capacity"], "state": "예약가능"}
...
def set_schedule():
    ...
    if program not in PROGRAMS or not date or capacity is None or state not in ("예약가능","예약마감","운항없음"):
